...
class McpTool(Tool):
            ...
            try:
                ...
                async def invoke_tool():
                    ...
                    async for client_session in client.get_client_session():
                        ...
                        tool_result=await client_session.call_tool(self.entity.name, tool_parameters, read_timeout_seconds=timedelta(seconds=60))
                    return ToolInvokeResult(name=self.entity.name, data=self.convert_content(tool_result.content),meta=tool_result.meta)
                return AsyncUtils.run_async(invoke_tool())
                # client = McpClient.build_client(server_url=self.server_url,mcp_config=self.entity.configs)
    ...
    def convert_content(self,content: list[mcp_types.TextContent
                                      | mcp_types.ImageContent
                                      | mcp_types.EmbeddedResource]) -> list[runtime_mcp_types.TextContent | runtime_mcp_types.ImageContent | runtime_mcp_types.EmbeddedResource]:
        ...
        for item in content:
            if isinstance(item, mcp_types.TextContent):
                ...
                converted_content.append(runtime_mcp_types.ImageContent(data=item.data, mimeType=item.mimeType,type=item.type))
            elif isinstance(item, mcp_types.EmbeddedResource):
                converted_content.append(runtime_mcp_types.EmbeddedResource(type=item.type,resource=runtime_mcp_types.TextResourceContents(text=item.resource.text)
                if isinstance(item.resource, mcp_types.TextResourceContents) else
                runtime_mcp_types.BlobResourceContents(blob=item.resource.blob, mimeType=item.resource.mimeType,uri=item.resource.uri)))
