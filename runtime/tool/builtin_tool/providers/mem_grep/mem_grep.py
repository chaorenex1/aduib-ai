...
class MemGrepTool(BuiltinTool):
    async def _invoke(self, tool_parameters: dict[str, Any], message_id: str | None = None) -> ToolInvokeResult:
        return await anyio.to_thread.run_sync(self._grep_sync, tool_parameters)

    def _grep_sync(self, tool_parameters: dict[str, Any]) -> ToolInvokeResult:
        try:
            query = tool_parameters.get("query")
            if not isinstance(query, str) or not query.strip():
                raise MemoryTreeError("query must be a non-empty string")
            data = CommittedMemoryTree.search_content(
                query=query,
