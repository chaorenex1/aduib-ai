...
class MemAddTool(BuiltinTool):
    async def _invoke(self, tool_parameters: dict[str, Any], message_id: str | None = None) -> ToolInvokeResult:
        return await anyio.to_thread.run_sync(self._add_sync, tool_parameters)

    def _add_sync(self, tool_parameters: dict[str, Any]) -> ToolInvokeResult:
        try:
            ...
            if storage_manager.exists(scoped_path):
                raise ValueError(f"memory file already exists: {path}")
            storage_manager.write_text_atomic(scoped_path, content)
            return ToolInvokeResult(name=self.entity.name, data={"path": path, "created": True})
