...
class ProjectPlanningState(MemoryContract):
    ...
    def apply_document_plans(self, document_plans: list[ProjectDocumentPlan], navigation_targets: list) -> None:
        self.document_plans = [item.model_copy(deep=True) for item in document_plans]
        self.navigation_targets = [item.model_copy(deep=True) for item in navigation_targets]
    ...
    def snapshot(self) -> dict:
        return {
            ...
            "navigation_targets": [
                item.model_dump(mode="python", exclude_none=True) for item in self.navigation_targets
            ],
