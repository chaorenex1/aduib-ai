...
class LongTermEmbeddingsMemory(MemoryBase):
    def __init__(
        ...
        self.text_splitter = RecursiveTextSplitter(chunk_size=self.chunk_size, chunk_overlap=self.chunk_overlap)
        self.pattern = r"^[\u2000-\u206F\u2E00-\u2E7F\u3000-\u303F!\"#$%&'()*+,./:;<=>?@^_`~]+"
        self.knowledge: KnowledgeBase = KnowledgeBase(rag_type="long-term")
        from component.vdb.vector_factory import Vector

        self.embeddings = self._get_embeddings()
        self.vector: BaseVector = Vector.get_vector_factory("milvus")().init_vector(
            knowledge=self.knowledge, attributes=[], embeddings=self.embeddings
        )
    ...
    def get_memory(self, query: str) -> list[dict]:
        ...
        try:
            ...
            documents = self.vector.search_by_vector(
                embed_query,
