...
async def query_cache(
    ...
    if cached_task is None:
        raise ApiHttpException(status_code=404, code="cache_not_found", message="Cache not found")
...
async def save_task(task_data: TaskDataRequest):
    ...
    return {"task_id": saved_task.id, "request_hash": saved_task.request_hash, "success": True}
...
async def get_task_history(
    ...
    return {"tasks": history, "total": len(history), "limit": limit, "offset": offset}
...
async def delete_task(task_id: int):
    ...
    if not success:
        raise ApiHttpException(status_code=404, code="task_not_found", message=f"Task {task_id} not found")
...
async def export_tasks(
    ...
    if format not in ["json", "csv"]:
        raise ApiHttpException(status_code=400, code="invalid_format", message="Format must be 'json' or 'csv'")

    tasks_data = TaskCacheService.export_tasks(format=format, mode=mode, backend=backend, limit=limit)

    if format == "json":
        return {"format": "json", "count": len(tasks_data), "tasks": tasks_data}
