...
from ..base.contracts import ConversationMessageRef, ConversationSourceView
...
class ConversationRepository(RepositoryBase):
        ...
        *,
        ...
        with get_db() as session:
            row = MemoryConversation(
                ...
                created_at=MemoryServiceBase.utcnow(),
                updated_at=MemoryServiceBase.utcnow(),
            )
            session.add(row)
    ...
    def _to_view(row: MemoryConversation) -> ConversationSourceView:
        return ConversationSourceView(
            ...
            created_at=MemoryServiceBase.isoformat(row.created_at),
            updated_at=MemoryServiceBase.isoformat(row.updated_at),
        )
